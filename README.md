...
2. **The actual portfolio** (`public/portfolio.html`), a self-contained, 7,500+ line single HTML file (steampunk-meets-Kerala-Chronicle aesthetic, 3D coverflow project carousel, canvas-rendered black hole accretion disk). This gets mounted into the 3D scene as a CSS3D "laptop screen" portal once you sit down.

So the Next.js app isn't the portfolio itself. It's a frame around it. The content lives in one static file, and the app is just the room you walk through to reach it.

Worth saying plainly, because the repo also has some scaffold and tooling debris left over from how it was built. Check [Known cruft](#known-cruft--cleanup-candidates) below before assuming everything here is load-bearing.
...
- **Three.js loads from a CDN at runtime**, not bundled via npm. `room-scene.ts` dynamically injects `<script>` tags for `three@0.128.0` and each addon (CopyShader, FXAAShader, EffectComposer, CSS3DRenderer, etc.) and waits on `window.THREE`. That's why there's no `three` entry in `package.json`.
- `usePrefersReducedMotion` gates the whole scene. Reduced-motion users presumably get a fallback path (`a11yMode`) instead of the WebGL room.
- `index.html` at the repo root is **not** the deployed entry point. It's a local-dev-only redirect page that polls `localhost:3000` and tells you to run `start.bat` if nothing's there. Netlify serves the Next.js app directly, so this file is just a leftover convenience for double-clicking on Windows.
...
| DB layer | Prisma + SQLite, scaffolded but not used by the portfolio itself (see below) |
...
Visit `localhost:3000` directly. Don't rely on the root `index.html` unless you're testing the Windows `start.bat` flow.
